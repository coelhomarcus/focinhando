...
const RegisterPet = () => {
  ...
  useEffect(() => {
    const checkUserComplement = async () => {
      ...
      try {
        const response = await fetch(`${API_BASE_URL}/user/complement`, {
          headers: {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const token = localStorage.getItem("authToken");
      const response = await fetch(`${API_BASE_URL}/pets/user`, {
        method: "POST",
        headers: {
          ...
          <div className="bg-white rounded-xl sm:rounded-2xl shadow-sm border border-gray-200 overflow-hidden">
            <div className="p-6 sm:p-8 border-b border-gray-200">
              ...
              <p className="text-gray-600 text-sm">
                ...
                <span className="text-red-500 font-semibold">*</span> são
                obrigatórios
              </p>
            </div>
            ...
            <form onSubmit={handleSubmit} className="p-6 sm:p-8">
              ...
              <div className="mt-4 sm:mt-6">
                ...
                <textarea
                  value={petForm.about}
                  onChange={(e) =>
                    setPetForm({ ...petForm, about: e.target.value })
                  }
                  required
                  rows={5}
              ...
              <div className="mt-6 sm:mt-8 flex flex-col sm:flex-row gap-3">
                ...
                <button
                  type="submit"
                  disabled={loading}
                  className={`flex-1 px-6 py-3 rounded-lg font-semibold transition-all text-sm sm:text-base ${
                    loading
